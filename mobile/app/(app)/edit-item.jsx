...
import {
  ScrollView, StyleSheet, Text, TouchableOpacity, View, Image, ActivityIndicator,
} from "react-native";
...
export default function EditItemScreen() {
  ...
  async function loadItem() {
    try {
      ...
      if (!found) { showAlert(t("error"), "Item not found", undefined, "error"); return; }
      setItem(found);
      ...
      showAlert(t("error"), err?.message ?? "Failed to load item", undefined, "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      showAlert(t("savedTitle"), t("itemUpdated"), [{ text: t("done"), onPress: () => navBack("/(app)/inventory") }], "success");
    } catch (err) {
  ...
  async function handleSaveBatch(batch, batchForm) {
    ...
    try {
      ...
      setBatches((prev) => prev.map((b) => b.id === batch.id ? updated.batch : b));
  ...
  const currentImageUri = imageUri
    ? imageUri
    : (!removeImage && item?.image_data ? `data:image/jpeg;base64,${item.image_data}` : null);
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.backBtn} onPress={() => navBack("/(app)/inventory")} activeOpacity={0.8}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={{ alignItems: "center", gap: 10 }}>
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                <TouchableOpacity style={styles.imagePickerBtn} onPress={pickImage} activeOpacity={0.8}>
                  ...
                  <Text style={styles.imagePickerBtnText}>{t("changeImage")}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.imageRemoveBtn} onPress={() => { setImageUri(null); setImageBase64(null); setRemoveImage(true); }} activeOpacity={0.7}>
                  <Text style={styles.imageRemoveBtnText}>{t("removeImage")}</Text>
                </TouchableOpacity>
              </View>
            </View>
          ) : (
        ...
        {/* Item fields */}
        <InputField label={t("itemNameLabel")} value={form.name} onChangeText={(v) => setField("name", v)} error={errors.name} />
        <InputField label={t("manufacturerOptLabel")} value={form.manufacturer} onChangeText={(v) => setField("manufacturer", v)} />

        <View style={styles.chipGroup}>
          <Text style={styles.chipGroupLabel}>{t("categoryChipLabel")}</Text>
          <View style={styles.chipRow}>
            {CATEGORY_VALUES.map((val, idx) => (
              <TouchableOpacity key={val} style={[styles.chip, form.category === val && styles.chipActive]} onPress={() => setField("category", val)} activeOpacity={0.75}>
                <Text style={[styles.chipText, form.category === val && styles.chipTextActive]}>{t(CATEGORY_KEYS[idx])}</Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        {form.category === "その他" && (
          <InputField label={t("customCategoryLabel")} value={form.customCategory} onChangeText={(v) => setField("customCategory", v)} error={errors.customCategory} />
        )}
        ...
        <View style={styles.chipGroup}>
          ...
          <View style={styles.chipRow}>
            {CONDITION_VALUES.map((val, idx) => (
              <TouchableOpacity key={val} style={[styles.chip, form.condition === val && styles.chipCondActive]} onPress={() => setField("condition", val)} activeOpacity={0.75}>
                <Text style={[styles.chipText, form.condition === val && styles.chipTextCondActive]}>{t(CONDITION_KEYS[idx])}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>

        <InputField label={t("priceChipLabel")} value={form.price} onChangeText={(v) => setField("price", v)} keyboardType="decimal-pad" error={errors.price} />
        <DatePickerField label={t("expiryOptionalLabel")} value={form.expiryDate} onChange={(v) => setField("expiryDate", v)} />
  ...
  return (
    <View style={styles.batchCard}>
      <View style={styles.batchCardHeader}>
        ...
        <Text style={styles.batchCardTitle}>{t("batchLabel")} · {batch.quantity} pcs</Text>
      </View>
      <InputField label={t("priceChipLabel")} value={form.price} onChangeText={(v) => setField("price", v)} keyboardType="decimal-pad" />
      <DatePickerField label={t("expiryOptionalLabel")} value={form.expiryDate} onChange={(v) => setField("expiryDate", v)} />
      <View style={styles.chipGroup}>
        <Text style={styles.chipGroupLabel}>{t("conditionChipLabel")}</Text>
        <View style={styles.chipRow}>
      ...
      <TouchableOpacity style={[styles.batchSaveBtn, saving && { opacity: 0.5 }]} onPress={() => onSave(form)} activeOpacity={0.85} disabled={saving}>
        <Text style={styles.batchSaveBtnText}>{saving ? t("saving") : t("saveBatch")}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function makeStyles(c) {
  return StyleSheet.create({
    ...
    imageRemoveBtn: { paddingVertical: 6, paddingHorizontal: 16, borderRadius: 20, backgroundColor: "rgba(239,68,68,0.1)", justifyContent: "center" },
    ...
    chip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: "rgba(129,128,126,0.1)", borderWidth: 1.5, borderColor: "transparent" },
    ...
    saveBtn: { backgroundColor: c.text, borderRadius: 20, paddingVertical: 18, alignItems: "center", marginBottom: 32, marginTop: 8 },
    saveBtnText: { color: c.bg, fontSize: 16, fontWeight: "700" },
    batchesHeader: { fontSize: 13, fontWeight: "700", color: c.textMuted, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 12, marginLeft: 4 },
    batchCard: { backgroundColor: c.surface, borderRadius: 16, padding: 16, marginBottom: 16 },
    batchCardHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 },
    batchCardTitle: { fontSize: 14, fontWeight: "700", color: c.text },
    batchSaveBtn: { backgroundColor: c.surface, borderRadius: 14, paddingVertical: 12, alignItems: "center", borderWidth: 1.5, borderColor: c.border, marginTop: 8 },
