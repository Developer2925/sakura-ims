...
export default function StartScanScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      return () => sub.remove();
    }, [foundItems, scanActive, showChoice])
  );

  function handleBack() {
    if (foundItems !== null) {
      hideSheet();
    } else if (!scanActive && !showChoice) {
      setShowChoice(true);
      setScanActive(false);
    } else {
  ...
  function goNoBarcode() {
    router.push({ pathname: '/(app)/scan-adjust', params: { noBarcode: 'true' } });
  }

  const showSheet = useCallback((items, barcode) => {
    setFoundItems(items);
  ...
  const handleProceed = useCallback(() => {
    router.push({ pathname: '/(app)/scan-adjust', params: { barcode: scannedBarcode } });
  }, [scannedBarcode]);
    ...
    <View style={styles.screen}>
      ...
      {showChoice && (
        ...
        >
          ...
          <TouchableOpacity activeOpacity={1} style={[styles.choiceSheet, { paddingBottom: Math.max(insets.bottom + 16 + 82, 126) }]}>
            <View style={styles.sheetHandle} />
            <Text style={styles.sheetTitle}>{t('useBarcodeChoiceTitle')}</Text>
            <Text style={styles.sheetSub}>{t('useBarcodeChoiceSub')}</Text>
            ...
            >
              ...
              <View style={styles.choiceText}>
                <Text style={styles.choiceBtnLabel}>{t('useBarcodeYes')}</Text>
                <Text style={styles.choiceBtnSub}>{t('useBarcodeYesSub')}</Text>
              </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => goNoBarcode()}
            >
              <View style={[styles.choiceIconWrap, { backgroundColor: getIconBoxColor('list', colors.isDark) }]}>
                <AppIcon name="list" size={24} />
              </View>
              <View style={styles.choiceText}>
                <Text style={styles.choiceBtnLabel}>{t('useBarcodeNo')}</Text>
                <Text style={styles.choiceBtnSub}>{t('useBarcodeNoSub')}</Text>
              </View>
              <AppIcon name="forward" size={18} />
            </TouchableOpacity>
          ...
          <Animated.View style={[styles.sheet, { transform: [{ translateY: slideAnim }] }]}>
            {notFound ? (
              <View style={styles.sheetRow}>
                <View style={[styles.thumb, { backgroundColor: getIconBoxColor('help', colors.isDark) }]}>
                  <AppIcon name="help" size={26} />
                </View>
                <View style={{ flex: 1, marginHorizontal: 14 }}>
                  <Text style={styles.sheetBrand}>{t('itemNotFound')}</Text>
                  <Text style={styles.sheetName}>{t('searchItem')}</Text>
                </View>
                <TouchableOpacity style={styles.sheetBtnRefresh} onPress={hideSheet} activeOpacity={0.85}>
                  <AppIcon name="refresh" size={20} />
                </TouchableOpacity>
              </View>
            ) : (
              <TouchableOpacity style={styles.sheetRow} onPress={handleProceed} activeOpacity={0.85}>
                <View style={[styles.thumb, { backgroundColor: getIconBoxColor('item', colors.isDark) }]}>
                  <AppIcon name="item" size={26} />
                </View>
                <View style={{ flex: 1, marginHorizontal: 14 }}>
                  <Text style={styles.sheetBrand} numberOfLines={1}>
                    {item?.category ?? t('useItem')}
                  </Text>
                  <Text style={styles.sheetName} numberOfLines={1}>{item?.name}</Text>
                </View>
                <TouchableOpacity style={styles.sheetBtn} onPress={handleProceed} activeOpacity={0.85}>
                  <AppIcon name="remove" size={22} color={colors.bg} />
                </TouchableOpacity>
              </TouchableOpacity>
            )}
          </Animated.View>
...
function makeStyles(c) {
  return StyleSheet.create({
    ...
    sheetTitle: { fontSize: 18, fontWeight: '800', color: c.text, marginBottom: 6 },
    sheetSub: { fontSize: 13, color: c.textMuted, marginBottom: 24 },
    choiceBtn: {
    ...
    choiceText: { flex: 1 },
    choiceBtnLabel: { fontSize: 15, fontWeight: '700', color: c.text, marginBottom: 3 },
    choiceBtnSub: { fontSize: 12, color: c.textMuted },
    cancelBtn: {
      marginTop: 16, paddingVertical: 16, alignItems: 'center',
      backgroundColor: 'rgba(129,128,126,0.1)', borderRadius: 16,
    },
    ...
    sheetBrand: { fontSize: 12, color: c.textMuted, fontWeight: '500', marginBottom: 3 },
