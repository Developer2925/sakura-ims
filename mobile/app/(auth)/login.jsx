...
export default function LoginScreen() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (result?.newUser) {
        router.push({ pathname: '/(auth)/google-onboarding', params: { email: result.email, firstName: result.firstName, lastName: result.lastName, googleId: result.googleId } });
        return;
      }
      ...
      showAlert(t("loginFailed"), err?.message ?? 'Google sign-in failed', undefined, 'error');
    } finally {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      showAlert(t("loginFailed"), err?.message ?? t("invalidCredentials"), undefined, "error");
    } finally {
      setLoading(false);
    }
  }
        ...
        <View style={styles.inner}>
          ...
          <TouchableOpacity style={styles.registerLink} onPress={() => router.push("/(auth)/register")} activeOpacity={0.7}>
            <Text style={styles.registerLinkText}>
              {t("noAccount")} <Text style={styles.registerLinkBold}>{t("createAccount")}</Text>
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
function makeStyles(c) {
  return StyleSheet.create({
    ...
    inner: { flex: 1, paddingHorizontal: 28, paddingTop: 60, paddingBottom: 40, justifyContent: "center" },
    logoWrap: { marginBottom: 36, alignItems: "center", gap: 6 },
    headline: { fontSize: 26, fontWeight: "800", color: c.text, letterSpacing: -0.5, marginTop: 8 },
    ...
    dividerLine: { flex: 1, height: 1, backgroundColor: c.border ?? "rgba(129,128,126,0.2)" },
    dividerText: { fontSize: 13, color: c.textMuted, fontWeight: "600" },
    googleBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 20, paddingVertical: 16, borderWidth: 1.5, borderColor: c.border ?? "rgba(129,128,126,0.25)", backgroundColor: c.surface },
